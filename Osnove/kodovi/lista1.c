...
void unos(int x, pozicija p) 
{
    ...
    q->next = p->next;  //varijabli "next" nove strukture dodjeljujemo vrijednost varijable "next" iz clana koji joj prethodi
    p->next = q;    //prethodni clan liste sada sadrzi adresu ovog novog clana koji se ubacio odmah iza njega
}
